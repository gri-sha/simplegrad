...
class Adam(Optimizer):
    ...
    When ``maximize=True`` the sign is flipped so the update ascends the
    objective (useful for reinforcement learning or contrastive objectives).

    Supports parameter groups, allowing different ``lr``, ``beta_1``,
    ``beta_2``, ``eps``, and ``maximize`` per group. Pass a list of dicts to
    ``param_groups``, each with a ``"params"`` key (a Module or a
    ``dict[str, Tensor]``) and optional per-group overrides:
    ...
    Args:
        ...
        param_groups: List of parameter group dicts with optional per-group
            overrides for ``lr``, ``beta_1``, ``beta_2``, ``eps``, and
            ``maximize``.
    """
    ...
    def step(self) -> None:
        ...
        for group in self.param_groups:
            ...
            for name, param in group["params"].items():
                ...
                update = lr * m_hat / (xp.sqrt(v_hat) + eps)
                if maximize:
                    param.values += update
                else:
                    param.values -= update
