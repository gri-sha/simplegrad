"""CNN benchmark: compare any combination of backends.

Architecture: Conv2d(3→32, k=3) → ReLU → MaxPool2d(2) →
              Conv2d(32→64, k=3) → ReLU → MaxPool2d(2) →
              Flatten → Linear(→512) → ReLU → Linear(→10)
...
Examples:
    python benchmarks/cnn.py --sg-cpu --torch-cpu
    python benchmarks/cnn.py --sg-cpu --sg-gpu
    python benchmarks/cnn.py --sg-cpu --torch-cpu --n-runs 50
...
def _build_sg_model(in_ch: int, img: int) -> sg.nn.Sequential:
    flatten_size = 64 * (img // 4) ** 2
    return sg.nn.Sequential(
        sg.nn.Conv2d(in_ch, 32, 3, pad_width=1),
        sg.nn.ReLU(),
        sg.nn.MaxPool2d(2, 2),
        sg.nn.Conv2d(32, 64, 3, pad_width=1),
        sg.nn.ReLU(),
        sg.nn.MaxPool2d(2, 2),
        sg.nn.Flatten(),
        sg.nn.Linear(flatten_size, 512),
        sg.nn.ReLU(),
        sg.nn.Linear(512, 10),
    )


def _build_torch_model(in_ch: int, img: int, dev: torch.device) -> tnn.Sequential:
    flatten_size = 64 * (img // 4) ** 2
    return tnn.Sequential(
        tnn.Conv2d(in_ch, 32, 3, padding=1),
        tnn.ReLU(),
        tnn.MaxPool2d(2, 2),
        ...
        tnn.MaxPool2d(2, 2),
        tnn.Flatten(),
        tnn.Linear(flatten_size, 512),
        tnn.ReLU(),
        tnn.Linear(512, 10),
    ).to(dev)


def bench_sg_cpu(*, batch, img, in_ch, n_runs, warmup):
    default_device("cpu")
    x = sg.Tensor(np.random.randn(batch, in_ch, img, img).astype(np.float32))
    model = _build_sg_model(in_ch, img)
...
def bench_sg_gpu(*, batch, img, in_ch, n_runs, warmup):
    import cupy as cp

    default_device("cuda:0")
    x = sg.Tensor(cp.random.randn(batch, in_ch, img, img).astype(cp.float32))
    model = _build_sg_model(in_ch, img)
...
def bench_torch(*, batch, img, in_ch, n_runs, warmup, device):
    dev = torch.device(device)
    model = _build_torch_model(in_ch, img, dev)
    x = torch.randn(batch, in_ch, img, img, device=dev, requires_grad=True)
...
def _build_configs() -> list[Config]:
    configs = []
    for img in [32, 64]:
        group = f"img {img}x{img}  in_ch=3  conv(3→32→64)  fc(→512→10)"
        for batch in [8, 32, 64]:
            configs.append(
                Config(
                    label=f"batch {batch}",
                    params={"batch": batch, "img": img, "in_ch": 3},
                    group=group,
                )
            )
    return configs
...
def main():
    ...
    parser.add_argument(
        ...
        default=None,
        metavar="PATH",
        help="custom log file path (default: benchmarks/logs/cnn_<datetime>.log)",
    )
    ...
    log_path = (
        default_log_path("cnn")
        if args.log_file is None
        else __import__("pathlib").Path(args.log_file)
    )
    log = setup_logging("cnn_benchmark", log_path)
