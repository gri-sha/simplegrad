"""Conv2d benchmark: compare any combination of backends.
...
def bench_sg_cpu(*, batch, in_ch, img, out_ch, kernel_size, stride, padding, n_runs, warmup):
    default_device("cpu")
    x = sg.Tensor(np.random.randn(batch, in_ch, img, img).astype(np.float32))
    conv = sg.nn.Conv2d(in_ch, out_ch, kernel_size, stride=stride, pad_width=padding)
...
def bench_sg_gpu(*, batch, in_ch, img, out_ch, kernel_size, stride, padding, n_runs, warmup):
    import cupy as cp
...
def bench_torch(*, batch, in_ch, img, out_ch, kernel_size, stride, padding, n_runs, warmup, device):
    dev = torch.device(device)
    x = torch.randn(batch, in_ch, img, img, device=dev, requires_grad=True)
    conv = tnn.Conv2d(in_ch, out_ch, kernel_size, stride=stride, padding=padding).to(dev)
